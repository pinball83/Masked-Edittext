...
public class MaskedEditText extends AppCompatEditText implements View.OnTouchListener, View.OnFocusChangeListener {
    ...
    private void initByAttributes(Context context, AttributeSet attrs) {
        ...
        if (TextUtils.isEmpty(mask) && TextUtils.isEmpty(notMaskedSymbol)) {
            ...
            System.out.println("masked edit text attributes, mask : " + mask + " charSequence : " + notMaskedSymbol);
    ...
    private void initListValidCursorPositions(String mask, String charSequence) {
        ...
        for (int i = 0; i < mask.length(); i++) {
            if (chars[i] == maskedSymbol) {
                System.out.println("MaskedEditText.initListValidCursorPositions index for input = " + i);
        ...
        System.out.println("firstAllowedPosition = " + firstAllowedPosition);
        System.out.println("lastAllowedPosition = " + lastAllowedPosition);
    }
    ...
    public void setInputType(int type) {
        ...
        if (type == InputType.TYPE_CLASS_NUMBER ||
            ...
            System.out.println("symbolExceptions = " + symbolExceptions);
    ...
    public void setMaskedText(String input) {
        if ((input != null) && (input.length() == listValidCursorPositions.size())) {
            ...
            System.out.println("set text into mask edittext");
            if (text != null) {
                for (int i = 0; i < mask.length(); i++) {
                    if (!listValidCursorPositions.contains(i)) {
                        buffer.insert(i, String.valueOf(mask.charAt(i)));
                        System.out.println("append " + buffer.toString() + " index " + i);
                    }
    ...
    private class MaskedInputFilter implements InputFilter {
        ...
        public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
            System.out.println("filter source = [" + source + "], start = [" + start + "], end = [" + end + "], dest = [" + dest + "], dstart = [" + dstart + "], dend = [" + dend + "]");

            if (textSetup) return source;

            if (!(source instanceof SpannableStringBuilder)) {
                System.out.println("Not SpannableStringBuilder");
                ...
                for (int i = start; i < end; i++) {
                    ...
                    if (charAllowed) {
                        ...
                        System.out.println("skip not allowed");
        ...
        private void skipSymbol(int index) {
            System.out.println("skipSymbol index = " + index);
        ...
        private void skipSymbolAfterDeletion(int index) {
            System.out.println("MaskedEditText.skipSymbolAfterDeletion");
            final int position = getNextAvailablePosition(index, true);
            setSelection(position);
        }

        private int getNextAvailablePosition(int index, boolean isDeletion) {
            System.out.println("getNextAvailablePosition index = [" + index + "], isDeletion = [" + isDeletion + "]");
            if (listValidCursorPositions.contains(index)) {
                System.out.println("MaskedInputFilter.getNextAvailablePosition contains");
                ...
                System.out.println("MaskedInputFilter.getNextAvailablePosition not contains");
                return findCloserIndex(index, isDeletion);
            }
        }

        private int findCloserIndex(int index, boolean isDeletion) {
            System.out.println("MaskedInputFilter.findCloserIndex");
        ...
        private boolean isCharAllowed(int index) {
            System.out.println("isCharAllowed index = [" + index + "]");
            final boolean result = index < mask.length() && mask.charAt(index) == notMaskedSymbol.toCharArray()[0];
            System.out.println("isCharAllowed result = " + result);
            return result;
        }
